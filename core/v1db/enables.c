...
void BrSetupLights(br_actor* world, br_matrix34* world_to_view, br_int_32 w2vt) {
    ...
    for (l = 0; l < v1db.enabled_lights.max; l++) {
         ...
         * appropriate
         */
        if ((light->type & BR_LIGHT_TYPE) == BR_LIGHT_POINT || (light->type & BR_LIGHT_TYPE) == BR_LIGHT_SPOT || light->volume.regions != NULL || light->type & BR_LIGHT_LINEAR_FALLOFF) {
        ...
        tvp->v.t = light->attenuation_q != BR_SCALAR(0.0) ? BRT_QUADRATIC : light->attenuation_l != BR_SCALAR(0.0) ? BRT_LINEAR
                                                                                                                   : BRT_CONSTANT;
        tvp++;
...
void BrSetupClipPlanes(br_actor* world, br_matrix34* world_to_view, br_int_32 w2vt, br_matrix4* view_to_screen) {
    br_matrix34 this_to_view, view_to_this;
    br_matrix4 screen_to_view, tmp4, screen_to_this, screen_to_this_i;
    int light_part = 0, i;
    br_int_32 clip_part;
    br_token_value tv[3];
    br_vector4 sp;
    ...
    for (i = 0; i < v1db.enabled_clip_planes.max; i++) {
         ...
         */
        // BrMatrix4Identity(&screen_to_this);
