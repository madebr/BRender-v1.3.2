...
br_error Device3DfxInitialise(br_device * self)
{
	...
	/*
	 * Check for presence of a 3Dfx board (this works with the new Glide)
	 */
 ...
 */
static br_token fullscreenInsignificantMatchTokens[] = {
		...
		(struct br_object_container *self, br_object *h, void *arg)
{
	 ...
	 */
	if (ObjectIsType(h, BRT_OUTPUT_FACILITY))

		if (((br_output_facility *)h)->fullscreen)
			return BrTokenValueComparePartial(tm->original, tm->query, fullscreenInsignificantMatchTokens);
		else
			return BrTokenValueComparePartial(tm->original, tm->query, windowedInsignificantMatchTokens);
	else
